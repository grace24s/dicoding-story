...
`;k.appendChild(T);const v=document.createElement("div");v.id="root-view";k.appendChild(v);const U=W(v);window.addEventListener("hashchange",()=>{setTimeout(()=>{const e=v.querySelector("main");e&&(e.setAttribute("tabindex","-1"),e.focus())},350)});let f=null;async function K(){if("serviceWorker"in navigator)try{const e=await navigator.serviceWorker.register("/sw.js");console.log("Service Worker registered",e),window.addEventListener("beforeinstallprompt",t=>{t.preventDefault(),f=t;const r=document.getElementById("installPrompt");r&&(r.hidden=!1)});const n=document.getElementById("btnInstall"),o=document.getElementById("btnDismissInstall");n&&n.addEventListener("click",async()=>{if(!f)return;f.prompt();const t=await f.userChoice;f=null,document.getElementById("installPrompt").hidden=!0}),o&&o.addEventListener("click",()=>{document.getElementById("installPrompt").hidden=!0});const a=document.getElementById("pushToggle");if(a){const t=await V(e);a.checked=t,a.addEventListener("change",async r=>{r.target.checked?await E(e):await q(e)})}}catch(e){console.error("SW register failed",e)}else console.log("Service Worker not supported")}async function V(e){try{return!!await e.pushManager.getSubscription()}catch{return!1}}async function E(e){try{const n=await e.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:I(D)}),o=localStorage.getItem("token");return await A({token:o,subscription:{endpoint:n.endpoint,keys:n.toJSON().keys}}),!0}catch(n){return console.error("subscribe failed",n),!1}}async function q(e){try{const n=await e.pushManager.getSubscription(),o=localStorage.getItem("token");return n&&(await B({token:o,endpoint:n.endpoint}),await n.unsubscribe()),!0}catch(n){return console.error("unsubscribe failed",n),!1}}document.addEventListener("click",e=>{e.target.id==="logoutBtn"&&(localStorage.clear(),location.hash="#/login")});window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),f=e;const n=document.getElementById("installPrompt");n&&(n.hidden=!1)});K();U.render();window.__app={subscribeUser:E,unsubscribeUser:q};
